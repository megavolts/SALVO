...
def process(writeFlag, plotTitle, survey_pts_csv_fn, ppk_pos_fn):
    ...
    ##Write out new file
    if writeFlag:
        out_csv_fn = filedialog.asksaveasfilename(
...
def HeightPlot(plotTitle, ppk_pos, survey_pts):
    ppk_pos["smoothed_height"] = (
        ppk_pos["height(m)"].rolling(window=5, min_periods=1).mean()
    )
    ...
    matching_End_rows = ppk_pos.loc[
        ppk_pos["DateTime"] == filtered_df["DateTime"].iloc[-1]
    ]
    ...
    matching_heights = ppk_pos.loc[
        ppk_pos["DateTime"].isin(matching_times), ["DateTime", "height(m)"]
    ]
    ...
    ax1.plot(
        ppk_pos["DateTime"], ppk_pos["height(m)"], color="blue", label="GPS Height"
    )
    ax1.fill_between(
        ppk_pos["DateTime"],
        ppk_pos["height(m)"] - 3 * ppk_pos["sdu(m)"],
        ppk_pos["height(m)"] + 3 * ppk_pos["sdu(m)"],
        color="lightblue",
        alpha=0.5,
        label="Uncertainty",
    ...
    ax1.scatter(
        matching_heights["DateTime"],
        matching_heights["height(m)"],
        color="red",
        marker="x",
        label="Recorded Depth",
        s=72,
    ...
    for i in range(len(ppk_pos) - 1):
        if ppk_pos["Q"].iloc[i] == 5:
            ax1.axvspan(
                ppk_pos["DateTime"].iloc[i],
                ppk_pos["DateTime"].iloc[i + 1],
                ymin=0,
                ymax=0.05,
                color="red",
                alpha=0.1,
    ...
    ax1.text(
        0.51, 0.044, text, transform=ax1.transAxes, fontsize=12, verticalalignment="top"
    )
...
def heightComp(plotTitle, outDF):
    ...
    for i in range(len(outDF) - 1):
        if outDF["Q"].iloc[i] == 5:
            ax1.axvspan(
                outDF["DateTime"].iloc[i],
                outDF["DateTime"].iloc[i + 1],
                ymin=0,
                ymax=0.05,
                color="red",
                alpha=0.1,
    ...
    ax2.plot(
        outDF["DateTime"], outDF["HeightDiff(m)"], color="red", label="Height Diff (m)"
    )
    ...
    ax3 = [
        fig.add_subplot(gs[3:5, i]) for i in range(3)
    ]  # Create three subplots in the last row, each taking one cell of a three-column layout
    ...
    for i, (ID, group) in enumerate(groups):
        ...
        ax3[i].set_ylim(
            3 * group["sdu(m)"].abs().min() * 0.85,
            3 * group["sdu(m)"].abs().max() * 1.15,
        )
        if i == 0:
        ...
        ax3[i].text(
            0.1,
            0.9,
            f"Mean= {3*round(group['sdu(m)'].abs().mean(), 2)} m",
            transform=ax3[i].transAxes,
            size=10,
            weight="bold",
        )
...
def xyComp(plotTitle, outDF):
    ...
    outDF["ggailatitude"] = survey_pts["latitude_a"].astype(str) + survey_pts[
        "latitude_b"
    ].astype(str)
    outDF["ggailatitude"] = outDF["ggailatitude"].astype(float)
    outDF["ggalongitude"] = survey_pts["Longitude_a"].abs().astype(str) + survey_pts[
        "Longitude_b"
    ].abs().astype(str)
    outDF["ggalongitude"] = outDF["ggalongitude"].astype(float)
    outDF["ggan_s_ind"] = "N"
    outDF["ggae_w_ind"] = "W"
    outDF["Lat"] = outDF.apply(
        lambda row: dms_to_dd(row["ggailatitude"], row["ggan_s_ind"]), axis=1
    )
    outDF["Lon"] = outDF.apply(
        lambda row: dms_to_dd(row["ggalongitude"], row["ggae_w_ind"]), axis=1
    )
    # Apply the geodesic function to each row to calculate 'Diff(m)'
    outDF["xyDiff(m)"] = outDF.apply(
        lambda row: calculate_distance_geodesic(
            row["Lat"], row["Lon"], row["latitude(deg)"], row["longitude(deg)"]
        ),
        axis=1,
    )
    ...
    for i, (ID, group) in enumerate(groups):
        ...
        ax3[i].set_xlabel(labels[i])
        ax3[i].set_ylim(
            3 * group["sdne(m)"].abs().min() * 0.85,
            3 * group["sdne(m)"].abs().max() * 1.15,
        )
        if i == 0:
        ...
        ax3[i].text(
            0.1,
            0.9,
            f"Mean= {3*round(group['sdne(m)'].abs().mean(), 2)} m",
            transform=ax3[i].transAxes,
            size=10,
            weight="bold",
        )
...
if __name__ == "__main__":
    ...
    survey_pts_csv_fn = filedialog.askopenfilename(
        title="Select survey points CSV file", filetypes=[("CSV files", "*.dat")]
    )
    ppk_pos_fn = filedialog.askopenfilename(
        title="Select rover .POS file", filetypes=[("POS files", "*.pos")]
    )
    ...
    plotTitle, ppk_pos, survey_pts, outDF = process(
        writeFlag, plotTitle, survey_pts_csv_fn, ppk_pos_fn
    )
