...
# Convert to UTC
survey_df["Timestamp"] = (
    pd.to_datetime(survey_df["Timestamp"], format="%Y-%m-%d %H:%M:%S.%f")
    - timedelta(hours=survey_tz)
    + timedelta(seconds=survey_dt)
)
# Round to nearest 200ms
rounding_factor = pd.to_timedelta(
    1000, unit="ms"
)  # Define rounding to the nearest 200 milliseconds
survey_df["Timestamp"] = survey_df["Timestamp"].apply(
    lambda x: x.round(rounding_factor)
)
...
ppk_pos_df["Datetime"] = pd.to_datetime(
    ppk_pos_df["Date"] + " " + ppk_pos_df["Time"], format="%Y/%m/%d %H:%M:%S.%f"
)

# Convert ppk_pos_df into X, Y and Z:
local_EPSG = 3338
xform = pyproj.Transformer.from_crs("4326", local_EPSG)
ppk_pos_df["X"], ppk_pos_df["Y"] = xform.transform(
    ppk_pos_df["Latitude"], ppk_pos_df["Longitude"]
)
...
ppk_pos_df = ppk_pos_df[
    (start_time < ppk_pos_df["Datetime"]) & (ppk_pos_df["Datetime"] < end_time)
]
...
ppk_pos_df["Timestamp"] = pd.to_datetime(
    ppk_pos_df["Datetime"], format="%Y-%m-%d %H:%M:%S"
)
