...
for SITE in ['ARM', 'BEO', 'ICE']:
    ...
    for ii, date in enumerate(sorted(fn_dict)):
        data_df = pd.read_csv(fn_dict[date][1])
        print(fn_dict[date][1].split('/')[-1], date, SITE, len(data_df), len(data_df.loc[data_df['SnowDepth']<=0]))
        data.append([fn_dict[date][1].split('/')[-1], date, SITE, len(data_df), len(data_df.loc[data_df['SnowDepth']<=0])])

pd.DataFrame(data).to_csv('/home/megavolts/depletion.csv')
