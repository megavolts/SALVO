...
root_dir = '/mnt/data/UAF-data/working_a/SALVO/00-SG27/1-Hz/'
os.chdir(root_dir)
...
def batch_decim(wkg_dir, smp=smp, glob_pattern='*.24o'):
    ...
    for file in glob.glob(glob_pattern):
        ...
        os.system(ShellString)
    return subdir
root_dir = os.path.join(root_dir, batch_decim(root_dir, smp,))
...
def batch_concat(wkg_dir, smp=smp, periodLength=7):
    ...
    fileDict = {int(file.split('sg27')[1][:3]):file for file in fileList if '_decim'+str('%ss.24o' %smp) in file}

    # Create 7 consecutive days bracket
    periods = []
    doy = np.array(sorted(fileDict.keys()))
    doy_start = doy[0]
    ...
    for period in periods:
        ...
        files = []
        for day in period:
            files += ['sg27' + str("%s0_decim%ss.24o" %(day, smp))]

        files = " ".join(files)
        outfile = 'sg27' + str("%s0_%s0_decim%ss.24o" %(period[0], period[-1], smp))
